...
@bp.route("/test-paytr-mode", methods=["GET"])
@platform_credentials_admin
def test_paytr_mode_gate():
    """GEÇİCİ: _paytr_test_mode_from_vault() doğrula — get-token/init YOK.

    Kullanım sonrası kaldırılacak. Secret değer döndürmez (paytr.mode secret değil).
    """
    import os
    ...
    db_row = fetch_one(
        ...
        SELECT is_configured,
               CASE
                 WHEN encrypted_value IS NULL OR BTRIM(encrypted_value) = ''
                 THEN FALSE ELSE TRUE
               END AS has_enc
        FROM public.platform_credentials
        WHERE credential_key = 'paytr.mode'
        """,
    )
    db_configured = bool(
        db_row
        and db_row.get("is_configured")
        and db_row.get("has_enc")
    )
    env_set = bool((os.environ.get("PAYTR_MODE") or "").strip())
    mode_raw = (get_credential("paytr.mode") or "").strip()
    # paytr.mode secret değil; yalnızca normalize etiket
    mode_label = mode_raw.lower() if mode_raw else "(empty)"
    if mode_label not in ("test", "live", "production", "prod", "(empty)"):
        mode_label = "(other)"
    if db_configured:
        source = "db"
    elif env_set:
    ...
    return jsonify(
        {
            "ok": True,
            "temporary": True,
            "endpoint": "/admin/test-paytr-mode",
            "test_mode": test_mode,
            "expects_test": test_mode == "1",
            "mode_label": mode_label,
            "source": source,
            "db_paytr_mode_configured": db_configured,
            "env_PAYTR_MODE_set": env_set,
            "paytr_init_called": False,
            "get_token_called": False,
            "note": "GEÇİCİ — kaldırılacak; gerçek ödeme/init yok",
        }
    )
