...
@bp.route("/test-paytr", methods=["GET"])
@platform_credentials_admin
def test_paytr_smoke():
    """GEÇİCİ: PayTR get-token smoke (test_mode=1). Secret değer ASLA döndürülmez.

    Kullanım sonrası kaldırılacak. Kart çekimi yok — yalnızca token isteği.
    """
    import base64
    import hashlib
    import hmac
    ...
    payload = {
        "ok": True,
        "gecici": True,
        "uyari": "GEÇİCİ test route — kullanım sonrası kaldırılacak. Secret değerler yanıtta yoktur.",
        "env": {
            "paytr.merchant_id": _mask_secret(mid),
            "paytr.merchant_key": _mask_secret(mkey),
            "paytr.merchant_salt": _mask_secret(msalt),
            "paytr.mode": _mask_secret(mode) if (mode or "").strip() else {
                "state": "BOŞ",
                "len": 0,
                "prefix": None,
    ...
    payment_amount = "100"
    merchant_oid = "smk" + str(int(time.time()))
    email = "smoke@payafin.com"
    ...
    user_basket = base64.b64encode(
        _json.dumps([["Payafin smoke", "1.00", 1]], ensure_ascii=False).encode()
    ).decode()
    no_installment = "0"
    max_installment = "0"
    currency = "TL"
    ...
    try:
        ...
        logger.exception("test_paytr_smoke")
    ...
    if j.get("reason") is not None:
        payload["paytr"]["reason"] = j.get("reason")
    tok = j.get("token")
    if tok:
